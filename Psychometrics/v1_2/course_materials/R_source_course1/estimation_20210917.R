...
g <- ggplot(dat, aes(x = d1, y = d2, label = val)) +
  geom_text(size = 10) +
  xlab("") +
  ylab("") +
  theme(
...
for (i in 1:10) {
  ...
  g <- ggplot(dat, aes(x = d1, y = d2, label = val, color = col)) +
    geom_text(size = 10) +
    xlab("") +
    ylab("") +
    theme(
...
mm %>%
  transform() %>%
  mutate(id = row_number(), val = X_data) %>%
  ggplot(aes(x = id, y = val)) +
  geom_point() +
  geom_line() +
  geom_hline(yintercept = mean(x), color = "red") +
  ylab("mean") +
  xlab("numbeer of samples")
...
c(mm, mm2) %>%
  transform() %>%
  mutate(id = rep(1:100, 2), val = X_data, n = rep(1:2, each = 100)) %>%
  mutate(n = factor(n, labels = c("n=10", "n=5"))) %>%
  ggplot(aes(x = id, y = val, color = n)) +
  geom_point() +
...
c(mm3, mm2, mm, mm4) %>%
  transform() %>%
  mutate(id = rep(1:100, 4), val = X_data, n = rep(1:4, each = 100)) %>%
  mutate(n = factor(n, labels = c("n=3", "n=5", "n=10", "n=20"))) %>%
  ggplot(aes(x = id, y = val, color = n)) +
  geom_point() +
  geom_hline(yintercept = mean(x)) +
  geom_line() +
...
vm %>%
  transform() %>%
  mutate(id = 1:nrow(mvec), val = X_data) %>%
  ggplot(aes(x = id, y = val)) +
  geom_point() +
  geom_line(color = "red") +
