...
for (Y in YEARS) {
  for (i in 1:NROW(tID)) {
    ...
    url1 <- paste0(baseURL, tmp, saralyURL, tID[i], "/") %>%
      ...
      str_replace(pattern = "https:/", replacement = "https://") %>%
      print()
    url2 <- paste0(baseURL, tmp, hitterURL, tID[i], "/") %>%
      str_replace_all(pattern = "//+", replacement = "/") %>%
      str_replace(pattern = "https:/", replacement = "https://") %>%
      print()
    url3 <- paste0(baseURL, tmp, pitcherURL, tID[i], "/") %>%
      str_replace_all(pattern = "//+", replacement = "/") %>%
      str_replace(pattern = "https:/", replacement = "https://") %>%
      print()
...
write_csv(baseball, file = "Baseball_All.csv")
baseball %>%
  mutate(Year2 = str_sub(Year, 1, 4)) %>%
  mutate(Year2 = as.numeric(Year2)) %>%
  filter(Year2 >= 2016) %>%
  select(-Year2) %>%
  write_csv(file = "BaseballDecade.csv")

read_csv("BaseballDecade.csv") %>%
  summary()

baseball %>%
  filter(Year == "2025年度") %>%
  write_csv(file = "Baseball25.csv")
# rm(list=ls())
