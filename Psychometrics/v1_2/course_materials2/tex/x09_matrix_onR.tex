...
表\ref{tbl::09_01}に示したのは，データ解析基礎でも扱った2000年代の野球選手のデータです\footnote{伴走サイト\url{https://stats.kosugitti.net/}より，サンプルデータにある「野球選手のデータ 10 年度分」を選んでください。ファイルはUTF-8形式で保存されています。}。それをプロジェクトフォルダに置き，\texttt{code:\ref{code:09_05}}のコードを実行してください。
