...
\begin{lstlisting}[language=c++,label={code:11_01},caption={データの読み込みと記述統計の確認}]
rm(list=ls())
library(tidyverse)
...
\begin{lstlisting}[language=c++,label={code:11_02},caption={散布図の作成}]
batter <- dat %>%
...
\section{線形回帰の実行}
それではここから回帰分析を行います。回帰分析は線形関数を当てはめるので\keyterm{線形モデル}{せんけいもでる}{linear model}と呼ばれますが，これの頭文字をとった\texttt{lm}関数が線形回帰モデルです。次の\texttt{code:\ref{code:11_04}}を実行してみましょう。
...
\texttt{code:\ref{code:11_04b}}を実行してみましょう。
\begin{lstlisting}[language=c++,label={code:11_04b},caption={回帰分析の実行}]
    plot(result)
\end{lstlisting}
...
\begin{Rscreen}{残差と予測値の確認}{out11_02}
	\begin{verbatim}
# A tibble: 329 × 4
...
\begin{Rscreen}{残差と予測値の要約統計量}{out11_02}
\begin{verbatim}
	height          weight         residuals             yhat       
...
さらにプロットを見てみましょう(図\ref{fig::11_02})。これは\texttt{ggplot}を使っての出力ではなくRオリジナルの出力例ですが(そのせいか少し色気のない出力ですが)，これを見て幾つか気づく点があると思います。
\begin{figure}[ht]
	\centering
...
\begin{Rscreen}{残差と予測値の相関関係}{out11_03}
\begin{verbatim}
	> cor(batter)
	height    weight     residuals         yhat
