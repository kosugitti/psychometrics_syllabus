...
さらにプロットを見てみましょう。これは\texttt{ggplot}を使っての出力ではなくRオリジナルの出力例ですが(そのせいか少し色気のない出力ですが)，これを見て幾つか気づく点があると思います。
...
\begin{lstlisting}[language=c++,label={code:ex_01},caption={データの準備と単回帰分析}]
rm(list=ls())
library(tidyverse)
dat <- read_csv("baseballDecade.csv", na="NA", 
        locale=locale(encoding="utf8")) %>% 
...
\paragraph{確認事項}：
\begin{itemize}
    \item 回帰式を$\hat{y} = a + bx$の形で書いてください
    \item 体重が1kg重くなると，身長は何cm高くなると予測されますか？
    \item $R^2$値はいくつですか？
\end{itemize}

\paragraph{subsection}{課題2：回帰診断プロットの確認}
\begin{lstlisting}[language=c++,label={code:ex_02},caption={回帰診断プロットの作成}]
plot(result1)
\end{lstlisting}
...
\paragraph{課題3：重回帰分析}
年俸を説明変数として，本塁打と安打を目的変数とする重回帰分析を実行してください。

\begin{lstlisting}[language=c++,label={code:ex_03},caption={重回帰分析用データの準備と実行}]
# 年俸，本塁打，安打のデータを準備
batter2 <- dat %>%
  dplyr::filter(position != "投手") %>%
  dplyr::select(salary, HR, Hit) %>%
  na.omit()
...
# 重回帰分析の実行（年俸で本塁打を予測）
result2 <- lm(HR ~ salary, data = batter2)
summary(result2)

# 重回帰分析の実行（年俸で安打を予測）
result3 <- lm(Hit ~ salary, data = batter2)
summary(result3)
\end{lstlisting}

\paragraph{確認事項}：
\begin{itemize}
    \item 年俸で本塁打を予測する回帰式を書いてください
    \item 年俸で安打を予測する回帰式を書いてください
    \item どちらのモデルの$R^2$値が高いですか？
\end{itemize}

\paragraph{課題4：標準化偏回帰係数の確認}
\begin{lstlisting}[language=c++,label={code:ex_04},caption={標準化データでの重回帰分析}]
# データを標準化
batter2_std <- scale(batter2) %>% as.data.frame()

# 標準化されたデータで重回帰分析
result_std1 <- lm(HR ~ salary, data = batter2_std)
result_std2 <- lm(Hit ~ salary, data = batter2_std)
...
\paragraph{確認事項}：
\begin{itemize}
    \item 標準化前後で$R^2$値は変化しましたか？
    \item 標準化偏回帰係数はそれぞれいくつですか？
\end{itemize}
