...
が得られます。どちらのルートでも結構ですから，自分のわかりやすい方で理解してください。一連の流れを図\ref{m24_two_way_root}にまとめてみました。
\begin{figure}[ht]
	\centering
	\begin{tikzpicture}
		\coordinate (A) at (1,1);
...
これらの図をみて残差が正規分布という仮定に従っているか，ということを確認することが重要です。こうした\keyterm{残差診断}{ざんさしんだん}{Residual Diagnostics}は，モデルの適合性や仮定の妥当性を評価しておくことが重要です。最尤法は確率モデルなのですから。
...
\paragraph{最尤推定の特徴}回帰分析における最尤推定について，正しい記述はどれですか？(複数選択可)
\begin{itemize}
	\item 正規分布を仮定した場合，最尤推定による推定値は最小二乗法による推定値と一致する
	\item 誤差項の分布が正規分布でない場合でも，常に最小二乗法と同じ結果になる
	\item 対数尤度関数を最大化することで，パラメータの推定値を得る
	\item 最尤推定では必ず誤差の標準偏差σも推定される
	\item 最尤推定は常に不偏推定量を与える
\end{itemize}

\paragraph{回帰分析の確率モデル}以下の式は何を表していますか？
$$ y_i ~ N(\beta_0 +\beta_1x_i, \sigma) $$
\begin{itemize}
	\item 誤差項の確率分布
	\item 回帰係数の確率分布
	\item 応答変数yiの条件付き確率分布
	\item 説明変数xiの確率分布
	\item 残差の確率分布
\end{itemize}
