...
g <- sample %>% select(height,weight) %>% na.omit %>% 
...
sample %>% select(Name,height,weight) %>% na.omit %>% tibble %>% head %>% 
    xtable::xtable(caption="データの一部",label="tab::04_04")

sample %>% dplyr::select(height,weight) %>% na.omit %>%  cor()
...
dat <- dat[v2,]
names(dat) <- c("V1","V2")
g <- ggplot(dat,aes(x=V1,y=V2))+geom_point()+ xlab("")+ylab("")
...
df <- rbind(df,c(20,20))
g <- df %>% transform() %>% ggplot(aes(x = X1, y = X2)) + geom_point() + xlab("")+ylab("")
cor(df)
...
g <- ggplot(dat,aes(x=V1,y=V2,color=judge,shape=judge))+scale_colour_colorblind()
...
g1 <- ggplot(dat,aes(x=X1,y=Y1))+geom_point()+scale_colour_colorblind()
g1

g2 <- ggplot(dat,aes(x=X1,y=Y1,shape=group,color=group))+geom_point()+ xlab("")+ylab("")+scale_colour_colorblind()
g2
