...
詳しくは \url{https://github.com/stan-dev/rstan/wiki/RStan-Getting-Started-(Japanese)}のサイトに説明がありますが\footnote{「RStan Getting Started (Japanese)」でググると出てきます。}\footnote{\citet{馬場201907}にも導入についての詳しい説明があります。}，Windowsユーザの場合はまず，\texttt{code:\ref{code::27_01}}を実行してください。
...
\begin{figure}[ht]
    ...
    \label{fig::27_01}
\end{figure}
この図の左側にあるのが，事後分布の形です。推定値である288.37や93.86は分布の平均値(EAP推定値)で，この分布のピーク(MAP推定値)ではないことに注意してください。MAP推定値が欲しい場合は，別途\texttt{bayestestR}パッケージを導入し，\verb|map_estimate|関数を実行する必要があります(\texttt{code:\ref{code::27_05}})。
...
\begin{Rscreen}{MCMCに関するメッセージ}{out27_04}
\begin{verbatim}
...
このように直接的に大きさを評価することもできますが，次のようなプロットを描くと特にわかりやすいでしょう(\texttt{code:\ref{code::27_07}})。
\begin{lstlisting}[language=c++,label={code::27_07},caption={周辺化して効果を見る}]
