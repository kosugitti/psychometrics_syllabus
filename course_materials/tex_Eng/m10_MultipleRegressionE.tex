...
Last time, under the title of "Regression Analysis," we created a forecast model based on the correlation of data. Prediction and control are the basics of science, so it's wonderful if we can do it.
...
However, the data we use is targeted at humans, so it does not fit perfectly like a theoretical model because it contains many errors. Still, if a roughly linear function holds (if it seems to be distributed in a straight line), we mentioned that it can often be useful.

% 今回は前回のモデルを少し複雑にしていきます。前回は$y=b_0+b_1x$で，説明される変数$y$に対して説明する変数$x$がひとつだけ，という簡単なモデルでした。今回は説明する変数が2つ以上のモデルを考えます。
This time, we are going to make the previous model a bit more complex. Last time, it was $y=b_0+b_1x$, a simple model where there was only one explanatory variable $x$ for the explained variable $y$. This time, we will consider a model where there are two or more explanatory variables.
...
Regression analysis with an increase in the number of explanatory variables is called \keyterm{Multiple Regression Analysis}\footnote{In contrast, a model with only one explanatory variable is often distinguished and called \keytermL{Simple Regression Analysis}.}. As the explanatory variables increase to $x_1, x_2\cdots x_m$, the coefficients correspondingly increase to $b_1, b_2,\cdots,b_m$. The intercept remains at $b_0$ regardless. This is represented by the following formula.
...
In general, we've made preparations up to $x_m$, but for instance, if there are two variables, it becomes $y_i = b_0 + b_1x_{1i} + b_2x_{2i} + e_i$. Please confirm carefully where the $i$ is attached and where it is not.
...
We have discussed the case of linear models when $y = b_0 + b_1 x$ (or the familiar form $y = ax + b$) which gives us a straight line. When there are two explanatory variables, it can be depicted in a 3D plane consisting of the $x_1$ axis, the $x_2$ axis, and the dependent variable $y$ (Figure \ref {fig :: 10_01}). When there are more than three explanatory variables, it is impossible to create a visual representation. However, there is no problem at all as an abstract/mathematical model. Also, even if the model is represented on a plane, it can appear as a straight line when viewed from a certain angle, so the names of linear regression or linear models generally do not change even when the number of explanatory variables increases.

\input{m10_MultipleRegression_escape_01}
...
The calculation to obtain the regression coefficient becomes more complex as the number of explanatory variables increases. While it does become more complicated, in the end, you can determine the coefficients based on the same concept as the least squares method, i.e., a criterion that sets the coefficients so as to minimize the variance of errors. Please note that other estimation methods and criteria, such as the Maximum Likelihood Method and Bayesian Estimation Method, which introduce concepts of inferential statistics and probabilistic models, will be introduced in the latter part of the course. These are all just different ways to find unknowns. I will not show the specific calculation formula here, but I would like you to know that you can get the answer with the same method as simple regression analysis if you use R.
...
Let's look at a concrete numerical example. Table \ref{tbl::10_01} contains data with one column added to Table \ref{tbl09_01}. This time, it includes the university GPA, which is the average grade of university students. It seems quite possible that high school interim scores and university entrance exam scores can affect the average university grades.

\input{m10_MultipleRegression_escape_02}
% このようなデータが得られたら，次にすることは，そう，可視化です。\textbf{データは図にする}が分析の基本だからです。
Once such data is obtained, the next thing to do is yes, visualization. This is because the basic rule of analysis is to \textbf{turn data into charts}.

\input{m10_MultipleRegression_escape_03}
...
Figure \ref{fig::10_02} presents a method for visualizing data using a $3 \times 3$ matrix. The rows represent the internal points, grades, and GPA, and the columns do too. The diagonal from the top left to the bottom right (in terms of cell address, it would be the nth row and nth column) contains diagrams that represent the distribution of data. Located at the bottom left are scatter plots for pairs of two variables. The top right holds their correlation coefficients. Ostensibly, the same information is in both the bottom left and top right, but one is visualized and the other is numerized.
...
Looking at this data, it seems that there is sufficient correlation among the three variables. Especially, if we consider the average university grade as the dependent variable, the correlation between the school report points and the average grade is 0.689, and the correlation between the entrance exam and the average grade is 0.664, and looking at the scatter plot, both seem to have a linear relationship from the lower left to the upper right. Therefore, let's try to perform multiple regression analysis considering the average grade as $y$, school report points as $x_1$, and entrance exam scores as $x_2$.

% Rでチョチョイと計算できるのですが，詳しくは次回に回すとして，結果として次のような回帰式が得られました。
Calculations can be quickly done in R, but I'll go into detail next time. As a result, I was able to obtain the following regression equation.
...
お気づきでしょうか。これを見ると，相関係数がきれいに$r=0.0$になっている箇所がありますね。そうです，残差$e_i$と2つの説明変数$x_1,x_2$，および残差$e$と予測値$\hat{y}$のところがゼロです。これは今回たまたまそうなったというのではなく，原理的にこのようになることが証明できます\footnote{照明については\textcite{kosugi2018}のPp.129--132をみてください。}。これがどういう意味を持つのか考えていきます。
...
Oh, both coefficients have turned negative. This means that both participating in club activities and having a part-time job are causing a drop in grades. Weren't club activities supposed to be good? Why has this reversal phenomenon occurred?

% 実は，このデータは$x_1$と$x_2$の間に相関関係があったのです($r_{12}=-0.4360$)。サークル活動をしているとバイトしている暇がない，あるいはその逆でしょうか。いずれにせよ負の相関があることをこのように解釈できます。
Actually, there was a correlation between $x_1$ and $x_2$ ($r_{12}=-0.4360$). It could be that if you are participating in club activities, you don't have time to work part-time, or vice versa. Either way, we can interpret this as there being a negative correlation.
...
Also, the correlation with the dependent variable is as follows: $r_{y1}=0.1421,r_{y2}=-0.5529$. This shows the same trend as the results of the simple regression analysis. It seems that people who spend more time in clubs have higher grades, and those who spend more time in part-time jobs have lower grades.
...
However, what needs to be noted is that in such a direct relationship as $x_1 \to y$, $x_2 \to y$, there are actually hidden influences mixed in. In this case, since there is a relationship between $x_1$ and $x_2$, there is a route of influence, $x_1 \to x_2 \to y$. This is called an "indirect effect".
...
The multiple regression analysis is showing the effects after controlling for indirect effects. It is not simply that "if you spend more time on club activities, your grades will drop, and if you spend a lot of time on part-time jobs, your grades will also drop". The correct interpretation is "if the time spent on part-time jobs is the same, the more time you spend on club activities, the lower your grades will be" and "if the time spent on club activities is the same, the more time you spend on part-time jobs, the lower your grades will be". Instead of drawing parallel conclusions as if both are independent, it's necessary to interpret conditionally.
...
Moreover, of course, if you haven't checked the hours worked part-time, you might conclude, "Club activities are good, and the data shows that grades will improve." When analyzing, if you don't comprehensively consider variables that are likely to influence the dependent variable, you will be led to wrong conclusions.
...
As we saw earlier, the simple correlation coefficient $r_{y1}$ is a mixture of the direct effects controlled (partial correlation coefficient) and indirect effects, so it must be well controlled and evaluated. I hope you now understand that when interpreting the results of multiple regression analysis, you must also be careful with the expression.
...
Also, if there is a strong correlation between the explanatory variables, the result of multiple regression analysis may become inaccurate. This problem is particularly referred to as the problem of \keyterm{multicollinearity}{multicollinearity}. Suppose $x_1$ and $x_2$ have a quite strong correlation, like $r_{12} = 0.9$ or close to $1.0$.
% $x_1 \simeq x_2$といってもいいような感じです。仮に$x_2$を$x_1$に置き換えてみたとすると，回帰式は次のようになります。
It would be fair to say that $x_1 \simeq x_2$. Suppose we try replacing $x_2$ with $x_1$, then the regression equation becomes as follows.
...
Looking at this, the regression coefficient for high school internal scores is 0.604, and the regression coefficient for entrance examination scores is 0.003. Looking at these two numbers, the score of the entrance examination is quite small, isn't it? Is the high school internal score 200 times more important than the entrance examination score? The answer is no. In this case, the dependent variable has a minimum value of 5.340 and a maximum value of 8.385. The explanatory variables are stretched (by multiplying coefficients) or shifted (by adding intercepts) to fit this dependent variable. The internal score has a minimum of 2.13 and a maximum of 3.90, so it's enough to stretch it similarly to fit the range of about 5-8. However, the entrance examination score ranges from a minimum of 460 to a maximum of 690. To make this a single digit number, like 5-8, you have to scale it down by about 1/100. Hence, the coefficient of $x_2$ is a small number with decimal points.
...
In this way, using raw data\footnote{This means unprocessed data. Here it is used to mean the data collected as it is.}, the units become different, so we cannot directly compare the partial regression coefficients to determine which one has a greater impact. So, what should we do? It was impossible to relatively compare because the units were different, which means it would be possible if there were no units. Yes, such an operation is called standardization, right?

% 説明変数も従属変数も単位がついていますから，これらを整えるためにそれぞれの変数を標準化します。標準化したデータセットは表\ref{tbl::10_03}のようになります。
Since both the explanatory variables and the dependent variables are unitized, we standardize each of these variables to arrange them. The standardized data set will look like Table \ref{tbl::10_03}.

\input{m10_MultipleRegression_escape_04}

% この標準化されたデータセットは，各変数平均が0分散が1となっており，このままで相対比較が可能です。これを使って回帰分析を行うと，結果は次のようになります。
This standardized dataset has each variable with a mean of 0 and a variance of 1, allowing for relative comparisons as is. Using this to perform regression analysis, the results will be as follows.
...
Next, the multiple regression formula for data standardized for each variable is
