...
So far, we have been explaining the mechanisms of regression analysis and \keyterm{multivariate regression analysis}{じゅうかいきぶんせき}{Multiple Regression Analysis} in words, but it's of no use if you can't calculate it yourself. We haven't specifically explained how to calculate the regression coefficient for multiple regression analysis, but when calculating with R, you can express it without distinguishing between simple regression and multiple regression, so let's learn it all at once here.

% \section{Rの環境再確認}
\section{Reconfirming the Environment of R}
% 第\ref{m03_introR},\ref{m04_usingR}章でRの使い方について解説してきました。思い出しながら進めていきましょう。
We have explained how to use R in Chapters \ref{m03_introR}, \ref{m04_usingR}. Let's proceed while recalling.

% \subsection{環境の準備(確認)}
\subsection{Preparation (Confirmation) of Environment}
...
Firstly, let's prepare the environment. We'll proceed assuming you've already installed R and RStudio\footnote{If you haven't yet, please catch up on Chapter \ref{m03_introR}. If you search the internet with keywords such as "RStudio installation", you'll find various websites that kindly guide you through the process.}.


% 次の3つのステップをたどって，実行の準備をしてください。
Please follow the next three steps to get prepared for execution.
% \paragraph{RStudioの起動}RStudioを起動してください。パッケージのインストールをするときなど，管理人権限が必要になるケースがありますので，Windowsユーザは管理人として実行するようにしたほうが良いかもしれません。
\paragraph{Starting RStudio} Please start RStudio. When installing packages, etc., there may be cases where administrator privileges are needed, so Windows users may want to run as an administrator.
...
\paragraph{Open Project} You can use the same project you created last time. You can create a new script file in the same folder as last time. Choose "Open project" from the menu (File$\to$Open Project) and select the ".Rproj" file in the target folder. That file holds the project information, and selecting it sets the working directory to that folder. You can check whether the project is open or not in the upper right of RStudio or from the menu bar.
...
\paragraph{Open R Script} Prepare an R script file to write today's code. Please go to File > NewFile > R Script and display the screen of the R script where nothing is written. If a blank file opens, there is no problem. You may proceed by writing in an Rmd file.

% これでスクリプトのところにコードを書いていく準備ができました。
Now you are ready to write some code in your script.
...
There should be plenty of data about statistics, to the extent that you get tired of entering it by hand. We will also use data loaded from a file this time. What needs to be careful when loading from a file is text encoding. We load ASCII data, but if the encoding is different, the characters will be garbled\footnote{If you do not know what it means, please refer to Appendix\ref{computer}.}. If your OS is Windows and you are using the non-standard world encoding Shift-JIS (CP-932), you may need to specify and load the world standard UTF-8.

% 今回，第\ref{m04_usingR}章の課題と同じ野球選手のデータを読み込んでみましょう。次の3行を実行してください。
This time, let's load the same baseball player data that we used in Chapter \ref{m04_usingR}. Please execute the following three lines.

\input{m11_Regression_onR_escape_01}

% \paragraph{コード解説}
\paragraph{Code Explanation}
\input{m11_Regression_onR_escape_02}

% \subsection{散布図の描画}
\subsection{Drawing Scatter Plots}
% データが手に入ったとき，最初にすることは可視化です。\textbf{データは図にする}という約束は忘れないでくださいね。
When you get the data, the first thing you do is visualize it. Don't forget the rule to 'turn the data into a chart'.
% 今回はさまざまな変数があるのですが，ここで打者の身長と体重のデータだけ抜き出して図にしてみたいと思います。
There are various factors this time, but I would like to extract only the data of the batter's height and weight and create a chart.

\input{m11_Regression_onR_escape_03}
% \paragraph{コード解説}
\paragraph{Code Explanation}
\input{m11_Regression_onR_escape_04}

% 結果的に図\ref{fig::11_01}のような図ができていればOKです。
If you end up with a figure like the one in Figure \ref{fig::11_01}, then it's OK.

\input{m11_Regression_onR_escape_05}

% さて1-3行目の操作ですが，ここでやりたかったことは\texttt{dat}オブジェクトの中から打者のデータに限って，身長と体重の変数を抜き出すことでした。
Now regarding the operation in lines 1-3, what I wanted to do here was to extract variables of height and weight specifically from the batter's data in the \texttt{dat} object.
...
These operations are implemented with the \texttt{filter} function and the \texttt{select} function of the \texttt{dplyr} package included in the \texttt{tidyverse} package. For the difference between the two, please refer to section \ref{dataHandling}, Pp.\pageref{dataHandling}.
% ともかくこれで欲しいデータの散布図が書け，どうやら正の相関がありそうだということがわかりました。
Anyway, with this we could draw the scatter plot of the data we wanted and we found out that there seems to be a positive correlation.
% 直線関係を仮定しても良さそうです。ちなみに次のコードを書くと\texttt{ggplot}で直線を書くことができますよ。
It seems okay to assume a linear relationship. By the way, you can draw a straight line with \texttt{ggplot} if you write the following code.

\input{m11_Regression_onR_escape_06}
% \paragraph{コード解説}
\paragraph{Code Explanation}
\input{m11_Regression_onR_escape_07}
...
Then, we will conduct a regression analysis. Regression analysis fits a linear function, so it is called a \keyterm{linear model}{せんけいもでる}{linerar model}, but the function named \texttt{lm}, which takes the initials of this, is a linear regression model. Let's execute the following \texttt{code:\ref{code:11_04}}.
\input{m11_Regression_onR_escape_08}

% \paragraph{コード解説}
\paragraph{Code Explanation}
\input{m11_Regression_onR_escape_09}
...
What's important here is the expression of the regression formula in the first line. In R, $y\sim x $ is used to express y = f(x). This is a standard way of expressing the relationship between the dependent variable y and the independent variable x by connecting them with $\sim$, it's a formula expression. In this case, we have used a function called lm to express this, but even when using a function other than a linear model to write the relationship between independent and dependent variables, it is expressed as y ~ x. Once you get used to this notation, it's easy to see that all analyses, in their basic form, are y = f(x), which is very helpful. In our case, we are predicting weight based on height.


% この分析の出力結果が出力\ref{RS:out11_01}の通りです。
The output of this analysis is as shown in output\ref{RS:out11_01}.
...
Various things are appearing, but let's check them in order. First, where it says \texttt{Residuals}, it represents descriptive statistical information of residuals $e_i$. It would be good to see how large the residuals are.

% 次の\texttt{Coefficientes}は係数という意味です。ここに回帰係数が出ています。少しみにくいかもしれませんが，表形式になっていて，\texttt{Estimate}と書かれたところが推定値，今回知りたい回帰係数の値になっています。
The \texttt{Coefficientes} here means coefficients. Here, the regression coefficients are shown. It might be a little hard to see, but it's in table format and the part where \texttt{Estimate} is written is the estimated value, which is the value of the regression coefficient we want to know this time.
% 回帰式は次のようになります。
...
When we see this, it seems that if the number of hits is about the same, the annual salary increases by 6.6 million yen per home run. Also, if the number of home runs is about the same, the annual salary increases by 530,000 yen per hit.
% またこのモデルの$R^2$は$0.378$ですから，予測モデルは全体の37\%ぐらいしか説明できておらず，あまり良いモデルとは言えないということがわかりました。
The $R^2$ of this model is $0.378$, which means that the predictive model can only explain about 37% of the whole, and it is understood that this is not a very good model.

% ヒットも本塁打も単位は「本」ですので，解釈も同じようにできて簡単かもしれません。あるいはヒットと本塁打は意味が違う，というのであれば直接の比較はおかしいことになります。
Both hits and home runs are counted in 'books', so it may be easy to interpret them in the same way. Alternatively, if a hit and a home run have different meanings, direct comparison may be inaccurate.
% ここでは後者の可能性を考えて，変数同士の単位が違うから標準化しよう，という方向で分析を進めたいと思います。
Here, considering the latter possibility, I would like to proceed with the analysis in the direction of standardizing because the units of the variables are different.

% データセットの各変数を標準化する必要がありますが，それには\texttt{scale}関数を使います。
It is necessary to standardize each variable in the dataset, for which we use the \texttt{scale} function.

\input{m11_Regression_onR_escape_11}


% \paragraph{コード解説}
\paragraph{Code Explanation}
\input{m11_Regression_onR_escape_12}

\input{m11_Regression_onR_escape_13}
...
Let's take a look at the results (Output \ref{RS:out11_04}). Can you understand how to read the estimated values? There is also a display like \verb|e-01| here, which means the same as $10^{-1}=\frac{1}{10}=0.1$. The result of the standardized regression analysis is as follows.
