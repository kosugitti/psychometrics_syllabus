...
g <- fit %>%
  ggplot(aes(x = theta)) +
  geom_density() +
  geom_vline(xintercept = pointEst$EAP, lty = 2, color = palette()[3]) + annotate("text",x=pointEst$EAP,y=4,label="A")+
  geom_vline(xintercept = pointEst$MAP, lty = 2, color = palette()[4]) + annotate("text",x=pointEst$MAP,y=3,label="B")+
  geom_vline(xintercept = pointEst$MED, lty = 2, color = palette()[2]) + annotate("text",x=pointEst$MED,y=2,label="C")+
  geom_segment(
    data = pointEst, aes(x = 0.0167, xend = 0.265, y = 0.7, yend = 0.7),
    arrow = arrow(ends = "both", angle = 90, length = unit(.2, "cm")), color = palette()[6]
  ) + annotate("text",x=pointEst$EAP+0.05,y=0.8,label="D", color = palette()[6])+
  geom_segment(
    data = pointEst, aes(x = 0.0290, xend = 0.291, y = 0.5, yend = 0.5),
    arrow = arrow(ends = "both", angle = 90, length = unit(.2, "cm")), color = palette()[1]
  )+ annotate("text",x=pointEst$EAP+0.1,y=0.6,label="E", color = palette()[1])
