...
\begin{figure}[ht]
    \centering
	\includegraphics[keepaspectratio,width=8cm]{../images/11_MultipleRegressionOnR/Rplot11_01.png}
    \caption{身長と体重のデータ}
    \label{fig::11_01}
\end{figure}

さて1-3行目の操作ですが，ここでやりたかったことは\texttt{dat}オブジェクトの中から打者のデータに限って，身長と体重の変数を抜き出すことでした。
これらの操作は\texttt{tidyverse}パッケージに含まれる\texttt{dplyr}パッケージの\texttt{filter}関数と\texttt{select}関数で実装されます。この両者の違いについては，セクション\ref{dataHandling},Page\pageref{dataHandling}を参照してください。
...
\begin{figure}[ht]
    \centering
	\includegraphics[keepaspectratio,width=12cm]{../images/11_MultipleRegressionOnR/Rplot11_02.png}
