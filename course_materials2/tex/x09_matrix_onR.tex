...
今から行う計算は，第\ref{x07_matrixModel}章でやったように，データ行列$\bm{X}$から，平均ベクトル$\bm{m}$，平均偏差行列$\bm{V}$，分散共分散行列$\bm{S}$，標準偏差を対角に持つ行列$\bm{Q}$，標準化スコア行列$\bm{Z}$，相関行列$\bm{R}$を作る，という手順です。数式とコードを示しますので，確認しながら進めてください。
