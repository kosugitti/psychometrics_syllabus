...
config <- result$gspace %*% result$cweights[[4]] %>% as.data.frame() %>% 
  rownames_to_column("judges") 
g1 <- config %>% ggplot(aes(x=D1,y=D2,label=judges))+geom_point()+geom_label()
g2 <- config %>% ggplot(aes(x=D1,y=D3,label=judges))+geom_point()+geom_label()
g3 <- config %>% ggplot(aes(x=D2,y=D3,label=judges))+geom_point()+geom_label()
library(patchwork)
g1+g2+g3
