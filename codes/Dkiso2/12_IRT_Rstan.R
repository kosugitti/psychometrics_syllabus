...
model_1pl <- stan_model("oneParameter.stan")
model_2pl <- stan_model("twoParameters.stan")
model_3pl <- stan_model("threeParameters.stan")

dataSet <- list(N = NROW(dat), M = NCOL(dat), resp = as.matrix(dat))
fit1.stanfit <- sampling(model_1pl, data = dataSet)
fit2.stanfit <- sampling(model_2pl, data = dataSet)
fit3.stanfit <- sampling(model_3pl, data = dataSet)
...
model_2pl_ver2 <- stan_model("twoParameters2.stan")
fit2.2 <- sampling(model_2pl_vers, data = dataSet)
